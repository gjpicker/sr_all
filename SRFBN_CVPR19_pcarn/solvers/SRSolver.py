...
from . import define_networks as define_networks
...
class SRSolver(BaseSolver):
    def __init__(self, opt):
        ...
        if self.is_train:
            ...
            self.netD = define_networks.define_D(input_nc=3,ndf=64,netD="n_layers",norm="instance").to(self.device)
            self.criterion_gan = define_networks.GANLoss(gan_mode="wgangp").to(self.device)
            # set cl_loss
            if self.use_cl:
            ...
            if optim_type == "ADAM":
                self.optimizer_G = optim.Adam(self.model.parameters(),
            ...
            if self.train_opt['lr_scheme'].lower() == 'multisteplr':
                ...
                self.scheduler_d = optim.lr_scheduler.MultiStepLR(self.optimizer_D,
                                                                self.train_opt['lr_steps'],
                                                                self.train_opt['lr_gamma'])
            else:
        ...
        if self.is_train:
            print("optimizer: ", self.optimizer_G)
    ...
    def train_step(self):
        ...
        lambx_r = self.train_opt["lambda_r"] 
        ...
        for i in range(self.split_batch):
            ...
            if self.use_cl:
                ...
                ###############
                loss_steps_gan = [self.criterion_gan_with_netD(sr, True) for sr in outputs]
                for step in range(len(loss_steps_gan)):
                    loss_sbatch_vgg += lambx_r * self.cl_weights[step] * loss_steps_gan[step]
            else:
                output = self.model(split_LR)
                loss_sbatch = self.criterion_pix(output, split_HR)
                loss_sbatch_vgg = self.criterion_gan_with_netD(output, True)

            loss_sbatch = loss_sbatch +loss_sbatch_vgg 
        ...
        if self.use_cl:
            loss_batch_d =self.train_d_step(split_HR,outputs)
        else:
            loss_batch_d =self.train_d_step(split_HR,output)


        self.model.eval()
        return loss_batch

    def criterion_gan_with_netD(self,data,is_real):
        ret = self.netD(data)
        loss_d = self.criterion_gan(ret,is_real)
        return loss_d

    def train_d_step(self, split_HR ,outputs  ):
        ...
        if True:
            ...
            if self.use_cl:
                loss_steps_f = [self.criterion_gan_with_netD(sr.detach(), False) for sr in outputs]
                loss_steps_r = [self.criterion_gan_with_netD(split_HR, True) for sr in outputs]
                for step in range(len(loss_steps_f)):
                    loss_sbatch +=lambx_r* self.cl_weights[step] * ( loss_steps_f[step] +  loss_steps_r[step] )

            else:
                loss_sbatch_f = self.criterion_gan_with_netD( outputs.detach(), False )
                loss_sbatch_r = self.criterion_gan_with_netD( split_HR, True )
                loss_sbatch = lambx_r*( loss_sbatch_f + loss_sbatch_r )

            if self.use_cl:
                for sr in outputs:
                    gradient_penalty_tmp,_ =define_networks.cal_gradient_penalty(netD=self.netD, real_data=split_HR.data, \
                        fake_data = sr.data,  device=self.device)
                    gradient_penalty +=  gradient_penalty_tmp
                gradient_penalty.backward(retain_graph=True)

            else:
                gradient_penalty,_ = define_networks.cal_gradient_penalty(netD=self.netD, real_data=split_HR.data, \
                        fake_data = outputs.data,  device=self.device)
                gradient_penalty.backward(retain_graph=True)
